...
def preprocess(path):
    ...
    print("in preprocess imglist====>",imglist)
    ...
    for i in range(len(imglist)):
        ...
        with open(os.path.join(img_path, 'bg_color.txt'), 'r') as bg_f:
            bg = bg_f.readline()
    ...
    print(f"len(img_all) = {len(matching_set['img_all'])}")
    cnt = 0
    ...
    print(f"cnt = {cnt}")
    print(f"matching_set['GT_pairs'] = {len(matching_set['GT_pairs'])}")
    return matching_set
...
data_path = sys.argv[1] + "/fragments"
...
# save path
root = sys.argv[2]+"/ori_dataset_all.pkl"
count = []
...
for n in range(len(sub_list)):
    ...
    preprocess(img_path)
    current_nums = len(matching_set['full_pcd_all'])
    print(f"len(matching_set['img_all']) = {len(matching_set['img_all'])}")
    count += 1
    if count == 1:
        print("------------------")
        # print(matching_set)
