...
# 下面注释的代码不知道干啥用的
# test_save_path = "../data/test_image"
...
def save_fragment(frags, save_path, frag_idx, background):
    ...
    f_pcd = frags.pcd
    f_trans = frags.trans
    cv2.imwrite(os.path.join(save_path, 'fragment_{}.png'.format(str(idx + 1).zfill(4))), f_img[:, :, :3])
...
def random_segmentation_circle(imgs):
    '''首先将imgs旋转角度得到roated_img,然后对旋转后的图片切分为两张小的碎片，再进行旋转'''
    ...
    bg = np.array([8, 248, 8], dtype=np.uint8)
    basic_cover = (img[:, :, 3] != 0)
    ...
    # new -> 分割得到的图片, pcd -> 轮廓点, p和n分别是被分开的两个碎片
    return new_p, new_n, pcd_p, pcd_n, rotate_matrix_p_nn, rotate_matrix_n_nn, bg, p_cover_area, n_cover_area, roated_img
...
if __name__ == '__main__':
    ...
    root = sys.argv[1]  # 所有包含一张图片的文件夹的路径
    segment_logic = sys.argv[2]
    ...
    if True or segment_logic == "circle_sample_V5_2":
        # 切割后的小碎片存储路径
        save_root = os.path.join(segment_logic, "fragments")
        # 切割过程存储路径
        process_root = os.path.join(segment_logic, "process")
        print(root)
        print(save_root)
        print(process_root)
        ...
        if os.path.exists(process_root) is False:
            os.makedirs(process_root)
        # 小碎片大小分布直方图
        areaJPG_root = segment_logic
        ...
        for m in range(len(img_list)):
            ...
            # 待切割原图，名称必须为1.jpg
            # todo 是否有必要修改
            img1 = cv2.imread(os.path.join(img_path, '1.jpg'), cv2.IMREAD_UNCHANGED)
            ...
            if not cur_res:
                continue
            # img_p img_n 分割后的两张碎片
            # pcd_p pcd_n 对应的轮廓点
            # temp_trans_p temp_trans_n 旋转矩阵
            # bg 背景颜色
            # p_cover_area n_cover_area
            # roated_img 旋转后的原始小碎片 用来生成切割过程的
            ...
            while any(x > area_thresh for x in this_image_area_list):
                ...
                for _ in range(15):
                    ...
                    Limit = 150
                    # print(cur_img_p.shape[0],cur_img_p.shape[1],cur_img_n.shape[0],cur_img_n.shape[1])
                    edge_scale_min_p = min(cur_img_p.shape[0] / cur_img_p.shape[1],
                                           cur_img_p.shape[1] / cur_img_p.shape[0])
                    edge_scale_max_p = max(cur_img_p.shape[1] / cur_img_p.shape[0],
                                           cur_img_p.shape[0] / cur_img_p.shape[1])

                    edge_scale_min_n = min(cur_img_n.shape[0] / cur_img_n.shape[1],
                                           cur_img_n.shape[1] / cur_img_n.shape[0])
                    edge_scale_max_n = max(cur_img_n.shape[1] / cur_img_n.shape[0],
                                           cur_img_n.shape[0] / cur_img_n.shape[1])
            ...
            for frag in fragment_list:
                pcd, img, trans = rotate_func(90,  frag.pcd, frag.img, pad_=20)
            ...
            print('保存中。。。len:', len(fragment_list))

            for idx in range(len(fragment_list)):
                save_fragment(fragment_list[idx], saved_path, idx, bg)
            with open(os.path.join(saved_path, 'bg_color.txt'), 'w') as f:
                f.write(str(bg)[1:-1])
